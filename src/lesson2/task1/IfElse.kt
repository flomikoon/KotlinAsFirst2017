...
fun ageDescription(age: Int): String {
    return if (age % 10 in 2..4 && age % 100 !in 5..20) "$age года"
...
fun segmentLength(a: Int, b: Int, c: Int, d: Int): Int  {
    if ((a<=c)&&(b<=d)) if (b-c<0) return -1 else return b-c
    if ((a>=c)&&(b>=d)) if (d-a<0) return -1 else return d-a
    if ((a<=c)&&(b>=d)) if (d-c<0) return -1 else return d-c
    if ((a>=c)&&(b<=d)) if (b-a==0) return -1 else return b-a
    return -1
}
